...
export default function Card({ item, variant = 'project' }) {
  ...
  return (
    <div style={{ background: 'var(--bg-card)', borderRadius: 'var(--radius-card)', padding: '1.5rem', border: '1px solid var(--border-color)', transition: 'all 0.3s ease', display: 'flex', flexDirection: 'column', height: '100%', gap: '1rem' }} className="card-hover">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ padding: '0.75rem', background: 'var(--accent-subtle)', color: 'var(--accent)', borderRadius: '12px' }}>
          <IconComponent size={24} />
        </div>
        {badge && (
          <span style={{ fontSize: '0.75rem', fontWeight: 'bold', padding: '0.25rem 0.75rem', borderRadius: 'var(--radius-pill)', background: badge.variant === 'success' ? '#10b98122' : 'var(--accent-subtle)', color: badge.variant === 'success' ? '#10b981' : 'var(--accent)' }}>
            {badge.label}
          </span>
        )}
      </div>
      ...
      {tags && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
          {tags.map(tag => <span key={tag} style={{ fontSize: '0.75rem', background: 'var(--bg-secondary)', padding: '0.25rem 0.5rem', borderRadius: '4px' }}>{tag}</span>)}
        </div>
      )}
      {actions && (
